...
class CNN_RNN(nn.Module):
    ...
    def _define_model(self):
        ...
        if self._hps['units'] == 'lstm':
            self._net_2 = nn.LSTM(self._features_size(), self._hps['hidden2'], bidirectional=False)

        else:
            self._net_2 = nn.GRU(self._features_size(), self._hps['hidden2'], bidirectional=False)
    ...
    def _features_size(self):
        ...
        return (out_pool_1 + out_pool_2) * self._hps['hidden1']
    ...
    def forward(self, x, msk_conv, msk_msg):
        ...
        union = torch.cat((x1, x2), dim=2)  # batch, hidden, features
        union = union.reshape(self._hps['batch_size'] * self._hps['conv_len'], -1)  # flattening

        dropped = self._dropout1(union)

        first_net = dropped.view(self._hps['conv_len'], self._hps['batch_size'], -1)
