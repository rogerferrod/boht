...
class ATT_RNN(nn.Module):
    ...
    def _define_model(self):
        ...
        self._positional_emb = PositionalEncoding(self._emb_bert_dim)
        self._attention = MultiHeadAttention(self._emb_bert_dim, self._hps['hidden1'], self._hps['heads'])
        self._norm = nn.LayerNorm(self._hps['hidden1'])

        if self._hps['units'] == 'lstm':
            self._net_2 = nn.LSTM(self._hps['hidden1'], self._hps['hidden2'], bidirectional=False)

        else:
            self._net_2 = nn.GRU(self._hps['hidden1'], self._hps['hidden2'], bidirectional=False)
    ...
    def forward(self, x, msk_conv, msk_msg):
        ...
        if self._hps['positional']:
            bert_emb = self._positional_emb(bert_emb)
        ...
        att = self._attention(bert_emb, att_msk)
        att_emb = self._norm(att)
        ...
        first_net = first_net.view(self._hps['conv_len'], self._hps['batch_size'], -1)
    ...
    def fit(self, x, y, msk_conv, msk_msg):
        ...
        if self._global_step % self._hps['save'] == 0:
            ...
            self._summary_writer.add_histogram('att_out/bias', self._attention.o_proj.bias, self._global_step)
            self._summary_writer.add_histogram('att_out/weight', self._attention.o_proj.weight, self._global_step)
