...
class BERT_RNN(nn.Module):
    ...
    def _define_model(self):
        ...
        if self._hps['units'] == 'lstm':
            self._net_2 = nn.LSTM(self._hps['emb_dim'], self._hps['hidden2'], bidirectional=False)

        else:
            self._net_2 = nn.GRU(self._hps['emb_dim'], self._hps['hidden2'], bidirectional=False)

        self._fc = nn.Linear(self._hps['hidden2'], 2)
