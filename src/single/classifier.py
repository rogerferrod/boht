...
def _compute_metrics(y_test, y_pred, out):
    ...
    out.write('Precision (macro) ' + str(prec) + '\n')
    out.write('Recall (macro) ' + str(rec) + '\n')
    out.write('F1 score (macro) ' + str(f1) + '\n')
    out.write('\n')
    out.write('Precision (weighted) ' + str(prec_w) + '\n')
    out.write('Recall (weighted) ' + str(rec_w) + '\n')
    out.write('F1 score (weighted) ' + str(f1_w) + '\n')
    out.write('\n')
    out.write('Precision ' + str(prec_m) + '\n')
    out.write('Recall ' + str(rec_m) + '\n')
    out.write('F1 score ' + str(f1_m) + '\n')
    out.write('\n')
...
def test_mode(out):
    ...
    for batch in testing_generator:
        pred_y = model.predict(batch.x, batch.msk_conv, batch.msk_msg, no_batch=True)
...
def demo_mode(out):
    ...
    for batch in testing_generator:
        ...
        predictions[batch.id] = tuple(zip(batch.original, pred_y, true_y))

    print('Writing results...')
    out.write('id;text;pred;gold\n')
    for k, val in predictions.items():
        for msg in val:
            ...
            text = ' '.join(text)
            out.write(k + ';' + text + ';' + str(msg[1]) + ';' + str(msg[2]) + '\n')
...
if __name__ == "__main__":
    ...
    parser.add_option("--demo_data", help='demo data folder', action="store", type="string", dest="demo",
                      default="../../input/single/run.bin")
    parser.add_option("-k", "--checkpoint", help='checkpoint', action="store", type="string", dest="checkpoint",
                      default="../../output/single/checkpoint/model.pt")

    (options, args) = parser.parse_args()

    torch.manual_seed(42)
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    available_models = ['RNN_RNN', 'BERT_RNN', 'ATT_RNN', 'ATT_ATT']
