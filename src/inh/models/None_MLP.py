...
class None_MLP(nn.Module):
    def __init__(self, device, hps, loss_weights):
        super(None_MLP, self).__init__()
        self._hps = hps
        self._device = device
    ...
    def _define_model(self):
        ...
        self._net_2 = nn.Sequential(
            nn.Linear(self._hps['emb_dim'], self._hps['hidden1']),
            nn.ReLU(),
            nn.Linear(self._hps['hidden1'], self._hps['hidden2']),
            nn.ReLU(),
        )
    ...
    def forward(self, x, msk_conv, msk_msg):
        ...
        msk_msg = msk_msg.reshape(self._hps['msg_len'], self._hps['batch_size'] * self._hps['conv_len']).unsqueeze(
            -1)
        net_in = embed_x * msk_msg  # remove padding
        first_net = self._pointwise_max(net_in)

        first_net = first_net.view(self._hps['conv_len'], self._hps['batch_size'], -1)

        # second net
        net_out = self._net_2(first_net)
